  ...
  Describe "Module $ModuleName" {
    Context "Module Documentation" {
      ...
      It "Has properties populated" {
        ...
        $Module.CompanyName.Length  | Should BeGreaterThan 0
        $Module.Copyright.Length    | Should BeGreaterThan 5
    ...
    ForEach-Object {
      ...
      Context "Command Documentation > $Command" {
       ...
       <#
        ...
        It "Command $($Command.Name) passes the PSScriptAnalyzer Rules" {
             (Invoke-ScriptAnalyzer -path "$psscriptroot\..\customfunctions\public\$command.ps1"  -ExcludeRule PSAvoidUsingConvertToSecureStringWithPlainText, psshouldprocess, PSAvoidUsingPlainTextForPassword).Count | Should Be 0
        }
<#
